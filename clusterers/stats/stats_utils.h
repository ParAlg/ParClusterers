...
inline gbbs::symmetric_graph<gbbs::symmetric_vertex, float> get_subgraph(const GbbsGraph& graph_, const std::vector<InMemoryClusterer::NodeId>& V, const parlay::sequence<gbbs::uintE>& labels, bool keep_ids = false){
    using Wgh = float;
...
inline gbbs::symmetric_graph<gbbs::symmetric_vertex, gbbs::empty> get_subgraph_unweighted(const GbbsGraph& graph_, const std::vector<InMemoryClusterer::NodeId>& V, const std::vector<int>& labels, bool keep_ids = false){
